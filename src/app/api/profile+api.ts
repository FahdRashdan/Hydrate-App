...
// `onboarding/profile.tsx` (PLAN.md Phase 2 — `/api/profile+api.ts`).
...
export async function DELETE(request: Request) {
  ...
  if (profile) {
    // Delete any bookings first to avoid foreign key constraint issues
    await db.delete(bookings).where(eq(bookings.customerId, profile.id));
